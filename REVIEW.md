...
| P0-5 | 8MiB 洪泛下无单次写入阻塞 >2s | `pty.rs::write_to_non_reading_child_does_not_block_the_caller` | 无需修复——实测 512×16KiB 全部送达、最慢单次写 54ms（conhost 持续 drain，代价是 ~0.5 MiB/s 吞吐而非阻塞）。测试保留为回归护栏 | ⤵️ 降级 P2（吞吐注记） |
