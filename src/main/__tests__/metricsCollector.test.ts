...
describe('metricsCollector sample interval (M8)', () => {
  it('derives the interval from measured captureSample cost with a safety margin', () => {
    // Empirical: captureSample is one powershell spawn running the combined
    // CPU+GPU counter script, measured at ~800ms wall on Windows. The interval
    // must stay above that so samples never overlap (the `running` guard is a
